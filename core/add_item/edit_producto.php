...
<body >
	...
	<div class="easyui-panel" title="Editar Producto" style="width:100%;max-width:400px;padding:10px 80px;">
		<form id="ff" method="post">
        <input type= hidden id =Id value = <?php print $_GET['Id']; ?>>
			<div style="margin-bottom:20px">
				<input class="easyui-textbox" required id="codigo_producto" style="width:100%" data-options="label:'Codigo:'" value =  <?php print $_GET['cod_producto'] ?>>
			</div>
			<div style="margin-bottom:20px">
				<input class="easyui-textbox" id="nombre_producto" style="width:100%" data-options="label:'Nombre:'" value= "<?php  print $_GET['nom_producto'] ?>">
			</div>	
            <div style="margin-bottom:40px">
             <select id=id_categoria    label="Categoria" style="width:100%">
                <?php retornarCategorias($_GET['cod_cat']);?>
             </select>

            </div>
		<div style="text-align:center;padding:5px 0">
			<input type=button class="btn btn-primary" value = Modificar onclick="recojerDatos(2,2)">
